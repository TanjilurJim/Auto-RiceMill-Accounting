...
const formatCurrency = (n: number | string) => Number(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
const FilterBar: React.FC<{
    ...
    return (
        <div className="grid grid-cols-1 items-center gap-4 md:grid-cols-2 lg:grid-cols-3 print:hidden">
            {/* dates */}
            <div className="">
                <InputCalendar
                    label="From"
            ...
            <div className="mt-6 flex flex-col gap-2 md:flex-row">
                ...
                <button
                    onClick={reset}
                    className="bg-background items-center rounded-md border px-4 py-2 text-sm text-gray-700 shadow-sm hover:bg-gray-50"
                >
                    Reset
                </button>
            </div>
...
interface Row {
    header: string;
    accessor: string | ((row: Row, index?: number) => React.ReactNode);
    className?: string;
}

interface DayBookIndexProps {
    rows: Row[];
    totals: {
        in_qty: number | string;
        out_qty: number | string;
        stock_value: number | string;
        rent_bill: number | string;
    };
    byType: Record<
        string,
        {
...
export default function DayBookIndex({ rows, totals, byType, filters }: DayBookIndexProps) {
            ...
            <div className="h-full w-screen space-y-6 p-4 md:p-12 lg:w-full">
                ...
                <div className="bg-background rounded-lg p-4 shadow-sm sm:p-6">
                    ...
                    <div className="mt-6 hidden md:block">
                        <TableComponent columns={columns} data={rows} noDataMessage="কোন ভাউচার পাওয়া যায়নি" />
                    </div>
