...
interface SalarySlip {
  ...
  date: string;
  creator: { name: string };
  salarySlipEmployees: SalarySlipEmployee[] | undefined; // Safely handled as undefined
}
...
export default function SalarySlipIndex({ salarySlips }: { salarySlips: PaginatedSalarySlips }) {
  ...
  return (
    <AppLayout>
      <Head title="All Salary Slips" />
      <div className="bg-gray-100 p-4">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-xl font-semibold">Salary Slips</h1>
          <Link
            href="/salary-slips/create"
          ...
          <table className="min-w-full border-collapse text-[13px]">
            ...
            <tbody>
              {salarySlips.data.map((salarySlip, index) => {
                // Check if salarySlip.salarySlipEmployees is not undefined and has length
                const totalAmount = (salarySlip.salarySlipEmployees || []).reduce(
                  (sum, item) => sum + parseFloat(item.total_amount),
                  0
                );

                return (
                  <tr key={salarySlip.id} className="hover:bg-gray-50">
