...
// Update the path below to the correct relative location of fmtDateTime
...
export default function EditUser({ user, roles, userRoles }: { user: User; roles: Role[]; userRoles: number[] }) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `${t('us-edit')} ${user.name}`, href: `/users/${user.id}/edit` },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} title={`${t('us-edit-user')} – ${user.name}`}>
            <Head title={`${t('us-edit-user')} – ${user.name}`} />

            <div className="p-4 md:p-12">
                <PageHeader title={t('us-edit-user')} addLinkHref="/users" addLinkText={t('us-back')} />
                ...
                <form onSubmit={submit} className="bg-background text-foreground mt-3 space-y-6 rounded-lg border p-4">
                        ...
                        <Field
                            ...
                            input={
                                <select
                                    ...
                                    className="bg-background focus:ring-ring w-full rounded-md border p-2 outline-none focus:ring-2"
                                >
                                    <option value="active">{t('us-active')}</option>
                                    <option value="inactive">{t('us-inactive')}</option>
                                </select>
                            }
                        />
                        <Field
                    ...
                    <div className="space-y-2 rounded-md border p-3">
                        <div className="text-sm font-medium">{t('us-assign-roles')}</div>
                        {authUser?.id === user.id && !(authUser?.roles ?? []).some((r: any) => r.name === 'admin') ? (
                            <p className="text-muted-foreground text-xs">{t('us-cannot-change-own-roles')}</p>
                        ) : (
                    ...
                    <div className="space-y-3 rounded-md border p-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-sm font-medium">{t('us-trial')}</div>
                        </div>

                        <div className="text-muted-foreground text-xs">
                            {t('us-current-end')}:{' '}
                            <span className="text-foreground font-medium">{user.trial_ends_at ? fmtDateTime(user.trial_ends_at) : '—'}</span>
                        </div>
  ...
  */}
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-2">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button size="sm" onClick={applyPlan} disabled={applying}>
                                        {applying ? t('us-applying') : t('us-apply-change')}
                                    </Button>
                        ...
                        <p className="text-muted-foreground text-xs">
                            “+7/+30” adds days to the later of <em>now</em> or the current trial end. “Set custom date” overwrites the end date (end
                            of selected day).
                        </p>
                    </div>
