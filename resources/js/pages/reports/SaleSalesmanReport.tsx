...
export default function SaleSalesmanReport({
    entries,
    filters,
    company,
}: {
            ...
            <div className=" max-w-full space-y-4 p-4">
                <Card className="shadow-lg">
                    <CardHeader className="relative bg-background py-6 text-center">
                        ...
                        <div className="mt-4">
                            <h2 className="text-xl font-semibold underline">Salesman-wise Sales Report</h2>
                            {filters.year ? (
                                <p className="text-sm">
                        ...
                        <div className="absolute top-16 right-4 print:hidden">
                            <Link href={route('reports.sale.filter', { tab: 'salesman' })} className="text-sm text-blue-600 hover:underline">
                                Change Filters
                            </Link>
                        </div>
                    </CardHeader>
                    ...
                    <CardContent className="p-6">
                                    ...
                                    {entries.length ? (
                                        <>
                                                ...
                                                <tr key={i}>
                                                    ...
                                                    {filters.year ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <td className="border px-2 py-1">{r.salesman_name}</td>
                                            ...
                                            <tr className="bg-gray-100 font-semibold print:bg-white">
                                                <td colSpan={filters.year ? 1 : 6} className="border px-2 py-1 text-right">
                                                    Grand Total
                                                </td>
                                                {filters.year ? (
                                                    ...
                                                    <>
                                                        <td className="border px-2 py-1 text-right">{totalQty.toFixed(2)}</td> {/* Qty correct */}
                                                        <td className="border px-2 py-1"></td> {/* Unit empty */}
                                                        <td className="border px-2 py-1"></td> {/* Rate empty */}
                                                        <td className="border px-2 py-1 text-right">{totalAmount.toFixed(2)}</td>{' '}
                                                        {/* Amount correct */}
                                                    </>
                                                )}
                        ...
                        <div className="mt-4 flex justify-end gap-2 print:hidden">
                            ...
                            <a
                                href={route('reports.sale.export', { tab: 'salesman', type: 'pdf', ...filters })}
                                target="_blank"
                            ...
                            <a
                                href={route('reports.sale.export', { tab: 'salesman', type: 'xlsx', ...filters })}
                                className="inline-flex items-center gap-1 rounded-md border px-4 py-2 text-sm hover:bg-gray-100"
                            >
