...
export default function ProfitLossRevamp({ from_date, to_date, figures, byLedger, grouped, company }: Props) {
    ...
    const barData = useMemo(
        () => [
            { name: 'Income', value: Math.abs((figures?.sales ?? 0) + (figures?.otherIncome ?? 0)) },
        ...
        <AppLayout>
            ...
            <div className="mb-4 flex items-center justify-between gap-3 print:hidden">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-lg leading-tight font-semibold">{company?.company_name}</div>
                ...
                <div className="flex items-center gap-2">
                    <Link className="text-sm text-blue-600 underline-offset-2 hover:underline" href={route('reports.profit-loss.filter')}>
                        Change filters
                    </Link>
                ...
                <CardContent>
                    <div className="overflow-x-auto">
                        <table className="min-w-full border-collapse text-sm">
                            ...
                            <tbody>
                                {filtered.rows.map((r, i) => (
                                    <tr key={`${r.ledger}-${i}`} className={i % 2 ? 'bg-muted/20' : undefined}>
                                        ...
                                        <td
                                            className={`border px-3 py-2 text-right tabular-nums ${r.credits - r.debits >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                                        >
                            ...
                            <tfoot>
                                <tr>
                                    <td className="border px-3 py-2 font-medium" colSpan={2}>
                                        Total
                                    </td>
                                    ...
                                    <td
                                        className={`border px-3 py-2 text-right font-semibold tabular-nums ${filtered.total.net >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                                    >
