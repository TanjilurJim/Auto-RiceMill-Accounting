    ...
    return (
        <AppLayout title="Ledger Group Summary" header="Ledger Group Summary">
            ...
            <div className="absolute top-4 right-4 print:hidden">
                <Link href={route('reports.ledger-group-summary.filter')} className="text-sm text-blue-600 hover:underline">
                    Change Filters
                </Link>
            </div>
                ...
                <div className="mt-4 overflow-x-auto">
                    <table className="w-full border-collapse text-sm">
                        <thead className="sticky top-0 bg-gray-100 text-gray-700 shadow-sm print:relative">
                            <tr>
                                <th className="w-16 border px-2 py-1 text-left">SL</th>
                                <th className="w-56 border px-2 py-1 text-left">Ledger Name</th>
                                <th className="border px-2 py-1 text-left">Mobile</th>
                                <th className="w-28 border px-2 py-1 text-right">Debit (Dr)</th>
                                <th className="w-28 border px-2 py-1 text-right">Credit (Cr)</th>
                            </tr>
                        </thead>
                            ...
                            {data.map((grp, gIdx) => (
                                <React.Fragment key={grp.group_name}>
                                    ...
                                    <tr className="bg-slate-50 font-medium text-slate-800">
                                        <td colSpan={5} className="border px-2 py-1 tracking-wide uppercase">
                                    ...
                                    {grp.ledgers.map((l, i) => (
                                        <tr key={l.id} className={i % 2 ? 'bg-gray-50/50' : undefined}>
                                            <td className="border px-2 py-1">{i + 1}</td>
                                            <td className="border px-2 py-1">{l.account_ledger_name}</td>
                                            <td className="border px-2 py-1">{l.phone_number || '—'}</td>
                                            <td className="border px-2 py-1 text-right">{l.debit ? money.format(l.debit) : '—'}</td>
                                            <td className="border px-2 py-1 text-right">{l.credit ? money.format(l.credit) : '—'}</td>
                                        </tr>
                                    ))}
                                    ...
                                    <tr className="bg-slate-100 font-semibold text-blue-700">
                                        ...
                                        <td className="border px-2 py-1 text-right">{money.format(grp.total_debit)}</td>
                                        <td className="border px-2 py-1 text-right">{money.format(grp.total_credit)}</td>
                                    </tr>
                                </React.Fragment>
                            ))}

                            {/* ───── Grand total row ───── */}
                            <tr className="bg-blue-50 text-base font-bold text-blue-800">
                                <td colSpan={3} className="border px-2 py-2 text-right">
                                    Grand Total&nbsp;:
                                </td>
                                <td className="border px-2 py-2 text-right">{money.format(grand_total_debit)}</td>
                                <td className="border px-2 py-2 text-right">{money.format(grand_total_credit)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
