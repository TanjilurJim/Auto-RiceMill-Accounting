...
function validateForm(data: Partial<DataShape>) {
  ...
  // Required fields
  if (!data.account_ledger_name?.trim()) errs.account_ledger_name = 'Please enter a ledger name.';
  if (!data.ledger_type) errs.ledger_type = 'Please select a ledger type.';
  if (!data.debit_credit) errs.debit_credit = 'Please choose Debit or Credit.';
  if (!data.status) errs.status = 'Please select a status.';
  if (!data.account_group_input) errs.account_group_input = 'Please select an account group or group under.';

  // Optional, but validate if provided
  if (data.email && !emailRegex.test(data.email)) errs.email = 'Please enter a valid email address.';
  if (data.phone_number && !bangladeshMobileRegex.test(data.phone_number)) {
    errs.phone_number = 'Use a valid BD mobile number (e.g., 01XXXXXXXXX).';
  }
  ...
  if (data.opening_balance !== '' && data.opening_balance !== undefined) {
    ...
    if (Number.isNaN(n)) errs.opening_balance = 'Opening balance must be a number.';
    else if (n < 0) errs.opening_balance = 'Opening balance cannot be negative.';
  }
  ...
  if (data.account_group_input && !/^group_under-\d+$|^account_group-\d+$/.test(data.account_group_input)) {
    errs.account_group_input = 'Invalid account group selection.';
  }
...
export default function CreateAccountLedger({
  ...
  const { data, setData, post, processing, errors: serverErrors } = useForm<DataShape>({
    account_ledger_name: '',
    phone_number: '',
    email: '',
      ...
      <div className="bg-background min-h-svh">
        <div className="p-4 md:p-12">
          ...
          <AccountLedgerForm
            ...
            errors={mergedErrors} // ← show client + server errors together
