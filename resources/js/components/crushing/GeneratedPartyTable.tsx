...
const GeneratedPartyTable: React.FC<Props> = React.memo(
  ({ rows, units, errors, onAdd, onRemove, onPatch, flashMain }) => {
    ...
    const tableColumns = [
      ...
      },
      {
        ...
        accessor: (row: GeneratedRow, idx: number) => (
          <>
            ...
            />
            {err(`generated.${idx}.qty`) && (
              <p className="text-xs text-red-500">{err(`generated.${idx}.qty`)}</p>
            )}
          </>
        ),
      },
      {
        ...
        accessor: (row: GeneratedRow, idx: number) => (
          <>
            ...
            {err(`generated.${idx}.unit_name`) && (
              <p className="text-xs text-red-500">{err(`generated.${idx}.unit_name`)}</p>
            )}
          </>
        ),
      },
      ...
      // NEW: By-product rate (৳/unit) — editable for NON-main rows
      {
      ...
      // NEW: By-product total (৳) — read-only like Company table
      {
